...
const SAMPLE_FAQS = [
  {
    ...
    answer:
      "네, 7이 나오면 반드시 도둑을 다른 타일로 옮겨야 합니다. 같은 자리에 둘 수 없습니다.",
    tags: ["규칙"],
    references: [
      {
        type: "link",
        value: "https://boardgamegeek.com/boardgame/13/catan",
        label: "BGG 카탄 페이지",
      },
  ...
  },
  {
    ...
    answer:
      "네, 시나리오에 실패하더라도 해당 시나리오에서 획득한 경험치와 골드는 유지됩니다.",
    tags: ["규칙", "FAQ"],
    references: [
      { type: "text", value: "Official FAQ v2.1" },
      {
        type: "link",
        value: "https://boardgamegeek.com/boardgame/174430/gloomhaven",
        label: "BGG 글룸헤이븐",
      },
    ],
    likes: 12,
    ...
    comments: [
      {
        id: "c3",
        author: "DungeonCrawler",
        text: "이건 정말 중요한 정보네요. 처음에 몰라서 경험치 다 날릴 뻔했습니다.",
  ...
  },
  {
    ...
    answer:
      "큐어(Cure)는 치료제를 발견한 상태이고, 박멸(Eradicate)은 치료제 발견 후 해당 질병 큐브가 보드에 하나도 없는 상태입니다. 박멸되면 해당 질병은 더 이상 감염되지 않습니다.",
    tags: ["규칙", "FAQ"],
    references: [],
    likes: 5,
    locked: false,
...
const SearchIcon = (p) => (
  <Icon d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" {...p} />
);
...
const ImageIcon = (p) => (
  <Icon
    d="M21 15l-5-5L5 21M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z"
    {...p}
  />
);
...
const GlobeIcon = (p) => (
  <Icon
    d="M12 2a10 10 0 100 20 10 10 0 000-20zM2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10A15.3 15.3 0 0112 2z"
    {...p}
  />
);
...
const SunIcon = (p) => (
  <Icon
    d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42M12 7a5 5 0 100 10 5 5 0 000-10z"
    {...p}
  />
);
const MoonIcon = (p) => (
  <Icon d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z" {...p} />
);
    ...
    <div className="ref-form-item">
      <div className="ref-type-btns">
        {[
          ...
          <button
            key={tp.type}
            className={`ref-type-btn ${refData.type === tp.type ? "active" : ""}`}
            onClick={() =>
              onChange(index, {
                ...refData,
                type: tp.type,
      ...
      <div className="ref-form-inputs">
        {refData.type === "link" && (
          <>
            <input
              placeholder={t.langUrl}
              value={refData.value}
              onChange={(e) =>
                onChange(index, { ...refData, value: e.target.value })
              }
            />
            <input
              placeholder={t.langUrlLabel}
              value={refData.label || ""}
              onChange={(e) =>
                onChange(index, { ...refData, label: e.target.value })
              }
            />
          </>
        )}
...
function FAQFormModal({ faq, games, onSave, onClose, lang, t }) {
  ...
  const updateRef = (i, newRef) =>
    setRefs((prev) => prev.map((r, idx) => (idx === i ? newRef : r)));
  const removeRef = (i) =>
    setRefs((prev) => prev.filter((_, idx) => idx !== i));
  const addRef = () =>
    setRefs((prev) => [...prev, { type: "text", value: "", label: "" }]);
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      likes: faq?.likes || 0,
      locked: faq?.locked || false,
      lockedBy: faq?.lockedBy || null,
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <div className="form-tags">
            {DEFAULT_TAGS.map((tg) => (
              <button
                key={tg}
                className={`form-tag-btn ${tags.includes(tg) ? "active" : ""}`}
        ...
        <div className="form-group">
          ...
          <div className="ref-form-list">
            ...
            <button className="add-ref-btn" onClick={addRef} type="button">
              <PlusIcon size={14} /> {t.addRef}
            </button>
          </div>
        </div>
...
const ChatIcon = (p) => (
  <Icon d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" {...p} />
);
...
function TranslateButton({ faq, currentLang, onTranslated, t }) {
  ...
  return (
    <div className="translate-dropdown-wrap" ref={wrapRef}>
      <button
        className={`action-btn translate-btn ${translated ? "active" : ""}`}
        onClick={() => {
          if (translated) {
            setTranslated(null);
            onTranslated(null);
    ...
    <div className={`faq-card ${faq.locked ? "locked" : ""}`}>
      <div className="faq-card-header">
        ...
        <div className="faq-card-badges">
          {translation && (
            <span className="badge badge-translated">
              <GlobeIcon size={11} />{" "}
              {LANGUAGES.find((l) => l.code === translation.lang)?.flag}
            </span>
          )}
          {faq.locked && (
      ...
      <div className="faq-answer">
        <span
          style={{ color: "var(--success)", fontWeight: 600, marginRight: 6 }}
        >
          A.
        </span>
      ...
      {translation && (
        <div className="faq-translation">
          <div className="faq-translation-label">
            <GlobeIcon size={13} />
            {LANGUAGES.find((l) => l.code === translation.lang)?.flag}{" "}
            {LANGUAGES.find((l) => l.code === translation.lang)?.label}{" "}
            {t.translated}
          </div>
          <div
            className="faq-translation-q"
            style={{ fontSize: 12, color: "var(--text-muted)" }}
          >
            원문 Q. {faq.question}
          </div>
          <div
            className="faq-translation-a"
            style={{ fontSize: 12, color: "var(--text-muted)" }}
          >
            원문 A. {faq.answer}
          </div>
            ...
            if (ref.type === "image")
              return (
                ...
                >
                  ...
                  {ref.value && (
                    <img src={ref.value} alt={ref.label || "reference"} />
                  )}
                </div>
              );
            return (
        ...
        <div className="faq-tags">
          {game && (
            <span className="faq-tag">
              {game.icon} {getGameName(game)}
            </span>
          )}
          {faq.tags.map((tg) => (
            <span key={tg} className="faq-tag">
              #{tg}
            </span>
          ))}
        </div>
        <div className="faq-actions">
          {ENABLE_TRANSLATION && !isGloballyTranslated && (
            <TranslateButton
              faq={faq}
              currentLang={lang}
              onTranslated={setCardTranslation}
              t={t}
            />
          )}
          {ENABLE_TRANSLATION && isGloballyTranslated && (
            <button
              className="action-btn translate-btn active"
          ...
          <button
            className={`action-btn like-btn ${faq._liked ? "liked" : ""}`}
...
export default function App() {
  ...
  const handleLike = async (id) => {
    ...
    setFaqs((prev) =>
      prev.map((f) =>
        f.id === id ? { ...f, likes: newLikes, _liked: !wasLiked } : f,
      ),
    );
    setLikedIds((prev) => {
  ...
  const handleLock = async (id, by) => {
    await supabase
      .from("faqs")
      .update({ locked: true, locked_by: by })
      .eq("id", id);
    setFaqs((prev) =>
      prev.map((f) => (f.id === id ? { ...f, locked: true, lockedBy: by } : f)),
    );
  ...
  const handleUnlock = async (id) => {
    ...
    setFaqs((prev) =>
      prev.map((f) =>
        f.id === id ? { ...f, locked: false, lockedBy: null } : f,
      ),
    );
  ...
  const handleAddComment = async (faqId, comment) => {
    await supabase
      .from("comments")
      .insert({ faq_id: faqId, author: comment.author, text: comment.text });
    // Optimistic update
    setFaqs((prev) =>
      prev.map((f) =>
        f.id === faqId
    ...
    <>
      ...
      <div className="app-container">
          ...
          <div className="header-top">
            ...
            <div className="header-controls">
              ...
              >
                ...
                {theme === "auto" && (
                  <span style={{ fontSize: 10, opacity: 0.6 }}>A</span>
                )}
              </button>
              {/* Language Switcher */}
              {ENABLE_TRANSLATION && (
              ...
              {adminRevealed && (
                <button
                  className={`ctrl-btn ${isAdmin ? "active" : ""}`}
            ...
            <div className="settings-panel">
              <div className="settings-row">
                ...
                <input
                  ...
                  value={lockThreshold}
                  onChange={(e) =>
                    setLockThreshold(Math.max(1, parseInt(e.target.value) || 1))
                  }
                />
              </div>
            </div>
        ...
        <div className="game-tabs">
          <button
            className={`game-tab ${selectedGame === "all" ? "active" : ""}`}
          ...
          {games.map((g) => (
            <button
              key={g.id}
              className={`game-tab ${selectedGame === g.id ? "active" : ""}`}
              onClick={() => setSelectedGame(g.id)}
            >
              <span>{g.icon}</span> {getGameName(g)}{" "}
              <span className="tab-count">{gameCountMap[g.id] || 0}</span>
            </button>
          ))}
        </div>
        ...
        {activeTags.length > 0 && (
          <div className="tag-filters">
            {activeTags.map((tg) => (
              <button
                key={tg}
                className={`tag-chip ${selectedTag === tg ? "active" : ""}`}
          ...
          {loading ? (
            ...
            filteredFaqs.map((faq) => (
              <FAQCard
                key={faq.id}
                faq={faq}
                game={games.find((g) => g.id === faq.gameId)}
                isAdmin={isAdmin}
                ...
                lang={lang}
                t={t}
                globalTranslation={globalTranslations[faq.id] || null}
              />
            ))
          )}
        </div>
